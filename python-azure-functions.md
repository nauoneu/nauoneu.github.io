...
<!-- omit in toc -->
# Azure Function App (サーバーレス) でコードを実行する

Azure Function App はサーバーレスでコードを動かすことができるAzureのサービスで、AWSでいうところのLambda。<br>
お手軽にWebサービスが作れるので簡単な処理を実装するのには便利なサービスです。

MicrosoftのサービスですからVS Codeを使うといろいろなプラグインが利用できて便利です。<br>
なので開発環境はVS Code前提とします。<br>
簡単な開発の流れとしては、<br>
VS CodeにAzure FunctionsのExtensionを入れてローカル環境で開発＆デバッグ
VS CodeをAzureアカウントに接続して開発したコードをデプロイ
...
https://learn.microsoft.com/ja-jp/azure/azure-functions/functions-develop-vs-code?tabs=node-v3%2Cpython-v2%2Cisolated-process&pivots=programming-language-python
Pythonで記述するAzure FunctionsのV2 modelを試してみる
https://zenn.dev/choshicure/articles/364875750f8888
PowerShell のスクリプトが実行できない場合の対処方法(ExecutionPolicyによりローカル実行時にエラーになる)
